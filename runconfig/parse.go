...
func Parse(cmd *flag.FlagSet, args []string) (*Config, *HostConfig, *flag.FlagSet, error) {
	...
	// container path.
	//
	// So the solution to this is from the CLI, use CLIVolumeSpecs instead of
	// Volumes. This allows the daemon side to accurately perform the same
	// processing that would have occurred on the client (in the case of Linux).
	// For Windows, this is moot as a container path cannot include a colon in
	// it, so it will be parsed deterministically as a bind mount (assuming it
	// is valid, that is).
	config := &Config{
		Hostname:        hostname,
		...
		Image:           image,
		Volumes:         nil,
		CLIVolumeSpecs:  flVolumes.GetMap(),
