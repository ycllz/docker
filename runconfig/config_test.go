...
func TestDecodeContainerConfigVolumes(t *testing.T) {
	...
	// A single volume that looks like a bind mount passed in Volumes (not CLIVolumeSpecs), such as a REST API caller.
...
// correctly.
func callDecodeContainerConfig(volumes []string, inCLIVolumeSpecs bool) (*Config, *HostConfig, error) {
	var (
		b   []byte
		err error
		c   *Config
	...
	w := ContainerConfigWrapper{
		Config: &Config{
			CLIVolumeSpecs: map[string]struct{}{},
	...
	for _, v := range volumes {
		if inCLIVolumeSpecs {
			w.Config.CLIVolumeSpecs[v] = struct{}{}
		} else {
	...
	if c != nil && len(c.CLIVolumeSpecs) != 0 {
		return nil, nil, fmt.Errorf("CLIVolumeSpecs should be empty after calling DecodeContainerConfig")
	}
