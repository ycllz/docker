...
func TestCheckSystemDriveAndRemoveDriveLetter(t *testing.T) {
	...
	if path, err = CheckSystemDriveAndRemoveDriveLetter("z", driver); err != nil {
	...
	if path, err = CheckSystemDriveAndRemoveDriveLetter("AB", driver); err != nil {
	...
	if path, err = CheckSystemDriveAndRemoveDriveLetter(`\l`, driver); err != nil {
	...
	if path, err = CheckSystemDriveAndRemoveDriveLetter(`c:\`, driver); err != nil {
	...
	// Failure on d:
	if path, err = CheckSystemDriveAndRemoveDriveLetter(`d:`, driver); err == nil {
		t.Fatalf("c: should fail")
	}
