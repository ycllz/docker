...
func setDevices(s *specs.LinuxSpec, c *container.Container) error {
...
func setNamespace(s *specs.LinuxSpec, ns specs.Namespace) {
...
func delNamespace(s *specs.LinuxSpec, nsType specs.NamespaceType) {
	idx := -1
	for i, n := range s.Linux.Namespaces {
		if n.Type == nsType {
			idx = i
...
func setNamespaces(daemon *Daemon, s *specs.LinuxSpec, c *container.Container) error {
	// network
...
func setMounts(daemon *Daemon, s *specs.LinuxSpec, c *container.Container, mounts []container.Mount) error {
