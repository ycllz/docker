...
func addFiles(drv graphdriver.Driver, layer string, seed int64) error {
	...
	if err := rootfs.WriteFile(root, root.Join(root.Path(), "file-a"), randomContent(64, seed), 0755); err != nil {
		return err
	}
	if err := root.MkdirAll(root.Join(root.Path(), "dir-b"), 0755); err != nil {
		return err
	}
	if err := rootfs.WriteFile(root, root.Join(root.Path(), "dir-b", "file-b"), randomContent(128, seed+1), 0755); err != nil {
		return err
	}

	return rootfs.WriteFile(root, root.Join(root.Path(), "file-c"), randomContent(128*128, seed+2), 0755)
}
...
func checkFile(drv graphdriver.Driver, layer, filename string, content []byte) error {
	...
	fileContent, err := rootfs.ReadFile(root, root.Join(root.Path(), filename))
	if err != nil {
		return err
	}
...
func addFile(drv graphdriver.Driver, layer, filename string, content []byte) error {
	...
	return rootfs.WriteFile(root, root.Join(root.Path(), filename), content, 0755)
}
...
func addManyFiles(drv graphdriver.Driver, layer string, count int, seed int64) error {
	...
	for i := 0; i < count; i += 100 {
		...
		for j := 0; i+j < count && j < 100; j++ {
			...
			if err := rootfs.WriteFile(root, file, randomContent(64, seed+int64(i+j)), 0755); err != nil {
				return err
			}
		}
	}
	...
	for i := 0; i < count; i += 100 {
		...
		for j := 0; i+j < count && j < 100; j++ {
			...
			case 0:
				...
				if err := rootfs.WriteFile(root, root.Join(root.Path(), change.Path), randomContent(64, seed+int64(i+j)), 0755); err != nil {
					return nil, err
				}
			// Add file
			case 1:
...
func checkManyFiles(drv graphdriver.Driver, layer string, count int, seed int64) error {
	...
	for i := 0; i < count; i += 100 {
		...
		for j := 0; i+j < count && j < 100; j++ {
			file := root.Join(dir, fmt.Sprintf("file-%d", i+j))
			fileContent, err := rootfs.ReadFile(root, file)
			if err != nil {
				return err
			}
...
func addLayerFiles(drv graphdriver.Driver, layer, parent string, i int) error {
	...
	if err := rootfs.WriteFile(root, root.Join(root.Path(), "top-id"), []byte(layer), 0755); err != nil {
		return err
	}
	...
	if err := rootfs.WriteFile(root, root.Join(layerDir, "layer-id"), []byte(layer), 0755); err != nil {
		return err
	}
	if err := rootfs.WriteFile(root, root.Join(layerDir, "parent-id"), []byte(parent), 0755); err != nil {
		return err
	}
...
func checkManyLayers(drv graphdriver.Driver, layer string, count int) error {
	...
	layerIDBytes, err := rootfs.ReadFile(root, root.Join(root.Path(), "top-id"))
	if err != nil {
		return err
	}
	...
	for i := count; i > 0; i-- {
		...
		thisLayerIDBytes, err := rootfs.ReadFile(root, root.Join(layerDir, "layer-id"))
		if err != nil {
			return err
		}
		...
		layerIDBytes, err = rootfs.ReadFile(root, root.Join(layerDir, "parent-id"))
		if err != nil {
			return err
		}
	}
...
// readDir reads a directory just like rootfs.ReadDir()
