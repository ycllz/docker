...
type ApplyDiffProtoDriver interface {
	...
	ApplyDiff(id, parent string, diff io.Reader, imagePlatform string) (size int64, err error)
}
...
func (d *naiveDiffDriverWithApply) ApplyDiff(id, parent string, diff io.Reader, imagePlatform string) (int64, error) {
	b, err := d.applyDiff.ApplyDiff(id, parent, diff, imagePlatform)
	if err == ErrApplyDiffFallback {
		return d.Driver.ApplyDiff(id, parent, diff, imagePlatform)
	}
	return b, err
}
...
func (d *Driver) ApplyDiff(id string, parent string, diff io.Reader, imagePlatform string) (size int64, err error) {
	dir := d.dir(id)
