...
func (d *WindowsGraphDriver) Diff(id, parent string) (arch archive.Archive, err error) {
	...
	if d.info.Flavour == diffDriver {
		return nil, fmt.Errorf("Windows Diff Disk Driver: Diff() not implemented")
	} else if d.info.Flavour == filterDriver {
		...
		return nil, fmt.Errorf("Unknown Windows driver flavour: %d", d.info.Flavour)
	}
}
...
func (d *WindowsGraphDriver) CopyDiff(sourceId, id string, parentLayerPaths []string) error {
	...
	if d.info.Flavour == diffDriver && d.active[sourceId] != 0 {
		logrus.Infof("Committing active id %s", sourceId)
		...
		defer func() {
			...
			if err != nil {
				logrus.Warnf("Failed to activate %s: %s", sourceId, err)
			}
		}()
	}
