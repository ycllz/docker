...
func createLayer(ls Store, parent ChainID, layerFunc layerInit) (Layer, error) {
	...
	layer, err := ls.Register(ts, parent, nil)
	if err != nil {
		return nil, err
	}
...
func TestTarStreamStability(t *testing.T) {
	...
	layer1, err := ls.Register(bytes.NewReader(tar1), "", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	layer2, err := ls.Register(bytes.NewReader(tar2), layer1.ChainID(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRegisterExistingLayer(t *testing.T) {
	...
	layer2a, err := ls.Register(bytes.NewReader(tar1), layer1.ChainID(), nil)
	if err != nil {
		t.Fatal(err)
	}

	layer2b, err := ls.Register(bytes.NewReader(tar1), layer1.ChainID(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTarStreamVerification(t *testing.T) {
	...
	layer2, err := ls.Register(bytes.NewReader(tar2), "", nil)
	if err != nil {
		t.Fatal(err)
	}
