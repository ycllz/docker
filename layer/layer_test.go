...
func (tf *testFile) ApplyFile(root rootfs.RootFS) error {
	...
	if err := rootfs.WriteFile(root, fullPath, tf.content, tf.permission); err != nil {
		return err
	}
	return nil
}
...
func TestMountAndRegister(t *testing.T) {
	...
	b, err := rootfs.ReadFile(path2, path2.Join(path2.Path(), "testfile.txt"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStoreRestore(t *testing.T) {
	...
	if err := rootfs.WriteFile(pathFS, pathFS.Join(pathFS.Path(), "testfile.txt"), []byte("nothing here"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	b, err := rootfs.ReadFile(pathFS, pathFS.Join(pathFS.Path(), "testfile.txt"))
	if err != nil {
		t.Fatal(err)
	}
