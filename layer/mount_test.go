...
func TestMountSize(t *testing.T) {
	...
	if err := rootfs.WriteFile(pathFS, pathFS.Join(pathFS.Path(), "file2"), content2, 0755); err != nil {
		t.Fatal(err)
	}
...
func TestMountChanges(t *testing.T) {
	...
	if err := rootfs.WriteFile(pathFS, pathFS.Join(pathFS.Path(), "testfile1.txt"), []byte("mount data!"), 0755); err != nil {
		t.Fatal(err)
	}
	...
	if err := rootfs.WriteFile(pathFS, pathFS.Join(pathFS.Path(), "testfile4.txt"), []byte("mount data!"), 0644); err != nil {
		t.Fatal(err)
	}
