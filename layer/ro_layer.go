...
func (rl *roLayer) TarStreamFrom(parent ChainID) (io.ReadCloser, error) {
	...
	return rl.layerStore.drivers[string(rl.platform)].driver.Diff(rl.cacheID, parentCacheID)
}
...
func (rl *roLayer) Metadata() (map[string]string, error) {
	return rl.layerStore.drivers[string(rl.platform)].driver.GetMetadata(rl.cacheID)
}
