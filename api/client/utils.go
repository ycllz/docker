...
func (cli *DockerCli) clientRequest(method, path string, in io.Reader, headers map[string][]string) (*serverResponse, error) {
	...
	if err != nil {
		...
		if cli.tlsConfig == nil {
			return serverResp, fmt.Errorf("%v. Are you trying to connect to a TLS-enabled daemon without TLS?", err)
		}
	...
	serverResp.contentType = resp.Header.Get("Content-Type")
	serverResp.serverHeader = resp.Header.Get("Server")
...
func (cli *DockerCli) clientRequestAttemptLogin(method, path string, in io.Reader, out io.Writer, index *registry.IndexInfo, cmdName string) (io.ReadCloser, int, error) {
	cmdAttempt := func(authConfig cliconfig.AuthConfig) (io.ReadCloser, int, error) {
		...
		if err == nil && out != nil {
			...
			err = cli.streamBody(serverResp.body, serverResp.contentType, true, out, nil)
		}
		if err != nil {
...
func (cli *DockerCli) stream(method, path string, opts *streamOpts) (*serverResponse, error) {
	...
	return serverResp, cli.streamBody(serverResp.body, serverResp.contentType, opts.rawTerminal, opts.out, opts.err)
}
...
// serverOSFromServerHeader parses the HTTP header returned from the docker
// daemon to extract the OS of the server. Header format is Docker/Version (OS)
func serverOSFromServerHeader(serverHeader string) string {
	re := regexp.MustCompile(`.*\((.+)\).*$`)
	if matches := re.FindStringSubmatch(serverHeader); matches != nil {
		return matches[1]
	}
	return ""
}
