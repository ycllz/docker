...
func (d *Directive) possibleParserDirective(line string) error {
	...
	logrus.Debugf("[BUILDER] considering parser directive in %q", line)
	...
	if len(tecMatch) != 0 {
		for i, n := range tokenEscapeCommand.SubexpNames() {
			if n == "escapechar" {
				...
				logrus.Debugf("[BUILDER] escape set to '%s'", tecMatch[i])
				d.escapeSeen = true
		...
		if len(tpcMatch) != 0 {
			for i, n := range tokenPlatformCommand.SubexpNames() {
				if n == "platform" {
					...
					logrus.Debugf("[BUILDER] platform set to '%s'", tpcMatch[i])
					d.platformSeen = true
	...
	logrus.Debugf("[BUILDER] no more parser directives allowed from this point")
